// estas funciones hace peticiciones de http al backend
...
// estas 2 funciones anteriores pasan antes de estar autentificada es decir aun no tienes token 
...
export async function getProfile() {
  ...
  const response = await fetch("http://localhost:8000/api/users/profile/", { //tenemos que hacerle una peticion al backend 
   // lo que hace fetch es mandar una solicitud a esa url y esperra la respuesta que le da el backend 
    method: "GET",
    headers: {
      "Content-Type": "application/json",
